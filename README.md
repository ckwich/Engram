...
The smoke test writes, updates metadata, searches, reads, and deletes a
temporary `_engramd_smoke_*` memory through the daemon.
...
Daemon mode currently routes stable memory search, chunk/full reads, writes,
metadata updates, and deletes through `engramd`. Direct in-process MCP mode
remains the default unless `ENGRAM_DAEMON_URL` is set. This is not a
LanceDB/Kuzu backend switch and does not add hosted tenant authorization.
