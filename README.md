...
Use `preview_memory_chunks`, `preview_source_connector`, `preview_document_extraction`, `prepare_document_draft`, `prepare_visual_extraction_request`, or `preview_visual_extraction` when you want to inspect what Engram would ingest before any write happens. Document draft proposals and image/OCR requests or observations are evidence records, not trusted active memory, until a later explicit review path promotes them.
