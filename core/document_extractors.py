...
TEXT_SOURCE_TYPES = {"md", "markdown", "txt", "text", "transcript"}
...
def prepare_document_disassembly(
    *,
    ...
    """Prepare a local document text/image inventory without writing memory."""
    ...
    if normalized_source_type in TEXT_SOURCE_TYPES:
        return _prepare_text_disassembly(path, normalized_source_type)
    if normalized_source_type != "pdf":
        raise ValueError("prepare_document_disassembly currently supports source_type='pdf', 'markdown', or 'text'")
    ...
    payload = {
        ...
        "quality_seed": {
            ...
            "table_candidate_pages": [
                page["page_number"] for page in pages if page.get("table_candidate")
            ],
...
def _prepare_text_disassembly(path: Path, source_type: str) -> dict[str, Any]:
    source = _source_record(path, source_type)
    data = path.read_bytes()
    content = data.decode("utf-8", errors="replace")
    title = _text_title(content, path)
    document_record = prepare_document_record(
        title=title,
        source_uri=source["source_uri"],
    ...
    text_status = "text" if non_whitespace >= TEXT_PAGE_THRESHOLD_CHARS else ("low_text" if non_whitespace else "no_text")
    page = {
        "page_number": 1,
        ...
        "image_count": 0,
        "table_candidate": _looks_like_table(content),
    ...
    payload = {
        ...
        "quality_seed": {
            "page_count": 1,
            "pages_reported": 1,
            "text_pages": [1] if text_status == "text" else [],
            "low_text_pages": [1] if text_status == "low_text" else [],
            "no_text_pages": [1] if text_status == "no_text" else [],
            "image_pages": [],
            "table_candidate_pages": [1] if page["table_candidate"] else [],
            "visual_review_needed_pages": [],
        },
        "extraction_receipts": [
            {
                "tool": "text-disassembly",
                "command": ["read_text", str(path)],
                "returncode": 0,
                "stderr": "",
                "duration_ms": 0.0,
            }
    ...
    payload["artifact_manifest"] = build_document_artifact_manifest(payload)
    payload["artifact_manifest"]["resume"]["page_range"] = payload["document"]["page_range"]
    payload["artifact_manifest"]["resume"]["merge_strategy"] = "single_text_document"
    payload["artifact_manifest"]["resume"]["resume_token"] = None
    payload["visual_artifact_candidates"] = []
    payload["visual_extraction_request_arguments"] = None
    payload["visual_extraction_request"] = None
    return payload
...
def _source_record(path: Path, source_type: str) -> dict[str, Any]:
    data = path.read_bytes()
    media_type = _media_type(source_type)
    return {
...
def _text_title(content: str, path: Path) -> str:
...
def _page_records(
    ...
    for page_number in range(page_start, page_end + 1):
        ...
        table_candidate = _looks_like_table(text)
        records.append(
            {
...
def _visual_evidence_plan(
    ...
    for page in pages:
        ...
        page_capabilities = _page_visual_capabilities(page)
        requested_capabilities.update(page_capabilities)
        source_ref = {
            ...
            "requested_capabilities": page_capabilities,
        }
...
def _page_visual_capabilities(page: dict[str, Any]) -> list[str]:
    capabilities: set[str] = set()
    if page.get("text_status") in {"no_text", "low_text"}:
        capabilities.add("ocr_text")
    if int(page.get("image_count") or 0) > 0:
        capabilities.add("figure_description")
    if page.get("table_candidate"):
...
def _looks_like_table(text: str) -> bool:
    ...
    for line in str(text or "").splitlines():
        ...
        if not stripped:
            continue
        columns = [part for part in re.split(r"\s{2,}|\t+", stripped) if part.strip()]
        if len(columns) >= 3:
            table_like_rows += 1
