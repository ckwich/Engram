...
class MemoryFirewall:
    ...
    def record_memory_guardrail(
        ...
        *,
        ...
        """Record a redacted firewall event for a memory write guardrail decision."""
        issue_codes = [str(code) for code in guardrail.get("issue_codes") or [] if str(code).strip()]
        decision = str(guardrail.get("decision") or "").strip() or "allow"
        source_ref = {
        ...
        if isinstance(context, dict):
            for field in ("operation_kind", "operation_index", "transaction_id", "knowledge_pr_id", "draft_id"):
                if context.get(field) is not None:
                    source_ref[field] = context[field]
        event = {
            "event_id": stable_id(
                "firewall",
                {
                    "kind": "memory_guardrail",
                    "memory_key": source_ref["memory_key"],
                    "decision": decision,
                    "issue_codes": issue_codes,
                    "source_ref": source_ref,
                },
            ),
            "decision": "quarantine" if decision == "block" else "review_required",
            "matched_patterns": issue_codes,
            "evidence_allowed": decision != "block",
            "guidance_allowed": False,
            "source_ref": source_ref,
            "created_at": now_iso(),
        }
