...
def validate_knowledge_citation(citation: dict[str, Any]) -> list[str]:
    ...
    if level == "knowledge_pr" and not str(citation.get("knowledge_pr_id") or "").strip():
        errors.append("missing_knowledge_pr_id")
    if level == "memory_ci" and not str(citation.get("ci_run_id") or "").strip():
        errors.append("missing_ci_run_id")
    return errors
...
def _infer_level(citation: dict[str, Any]) -> str:
    ...
    if citation.get("edge_id") or citation.get("path_id"):
        return "graph"
    if citation.get("ci_run_id"):
        return "memory_ci"
    if citation.get("knowledge_pr_id"):
        return "knowledge_pr"
    return ""
