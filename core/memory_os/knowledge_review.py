...
def build_review_preparation(
    ...
    *,
    ...
    documents = _project_documents(list_records(ledger, "documents"), project=project)
    ...
    if not drafts and not receipts:
    ...
    return {
        ...
        "citations": normalize_knowledge_citations(
            [_document_citation(document) for document in cited_documents],
            default_source="memory_os",
        ),
        "omissions": [],
        "errors": errors,
        "source_reads": len(limited_drafts) + len(cited_documents) + len(receipts),
        "write_performed": False,
        "active_memory_write_performed": False,
    }
...
def _project_documents(records: list[dict[str, Any]], *, project: str) -> list[dict[str, Any]]:
    return [
        record
        for record in records
        if not str(record.get("project") or "").strip() or str(record.get("project") or "").strip() == project
    ]
