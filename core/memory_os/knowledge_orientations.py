...
def build_source_orientation(
    ...
    *,
    ...
    sources = _matching_records(list_records(ledger, "sources"), project=project, focus=focus)
    ...
    citations = _document_citations(summaries) or _source_citations(source_items)
    return _orientation_result(
        answer=answer,
        citations=citations,
        omissions=omissions,
        source_reads=len(sources) + len(limited_documents) + _related_read_count(summaries),
    )
...
def _matching_records(
    ...
    *,
    ...
    return [
        ...
        if _matches_project(record, project) and _matches_focus(record, focus)
    ]
...
def _matching_documents(
    ...
    *,
    ...
    matches = []
    for record in records:
        if not _matches_project(record, project):
            continue
...
def _matches_project(record: dict[str, Any], project: str) -> bool:
    ...
    return not record_project or record_project == project
