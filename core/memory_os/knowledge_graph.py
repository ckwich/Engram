...
def build_graph_evidence(
    ...
    *,
    ...
    edges = _rank_graph_edges(stored_edges + draft_edges)[: max(int(max_records), 1)]
    if not edges:
...
def _matching_edges(
    ...
    *,
    ...
    for edge in edges:
        ...
        if edge_project and edge_project not in project_values:
            continue
...
def _rank_graph_edges(edges: list[dict[str, Any]]) -> list[dict[str, Any]]:
    return sorted(edges, key=_graph_edge_rank)


def _graph_edge_rank(edge: dict[str, Any]) -> tuple[int]:
    edge_type = str(edge.get("edge_type") or "")
    source = str(edge.get("source") or "")
    status = str(edge.get("status") or "")
    if edge_type in CONTRADICTION_EDGE_TYPES and status != "draft":
        return (0,)
    if status == "active" and source == "document_intelligence.auto_graph":
        return (1,)
    if status == "active" and source.startswith("document_intelligence"):
        return (2,)
    if status == "draft" and source.startswith("document_intelligence"):
        return (3,)
    if status == "draft":
        return (4,)
    if source == "graph_proposal_batch":
        return (5,)
    if source == "legacy_related_to":
        return (6,)
    if source == "document_ingestion.structural":
        return (7,)
    return (8,)
