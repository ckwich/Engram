    ...
    try:
        ...
        record(
            ...
            "ok",
            {
                ...
                "backend_used": search_response.get("backend_used"),
                "primary_backend": search_response.get("primary_backend"),
                "fallback_used": search_response.get("fallback_used"),
                "fallback_reason": search_response.get("fallback_reason"),
                "memory_os_retrieval_status": search_response.get(
        ...
        benchmark_catalog = client.list_memory_benchmark_suites({"suite_id": "smoke"})
        _raise_for_daemon_error("list_memory_benchmark_suites", benchmark_catalog)
        suites = [suite for suite in benchmark_catalog.get("suites") or [] if isinstance(suite, dict)]
        if not any(suite.get("suite_id") == "smoke" for suite in suites):
            raise _SmokeFailure(
                "list_memory_benchmark_suites",
                "benchmark_suite_missing",
                "Daemon did not advertise the smoke benchmark suite.",
                benchmark_catalog,
            )
        record("list_memory_benchmark_suites", "ok", {"suite_id": "smoke"})

        benchmark_run = client.run_memory_benchmark(
            {"suite_id": "smoke", "seed": 42, "persist": True}
        )
        _raise_for_daemon_error("run_memory_benchmark", benchmark_run)
        run_id = str(benchmark_run.get("run_id") or "")
        if benchmark_run.get("summary", {}).get("status") != "pass" or not run_id:
            raise _SmokeFailure(
                "run_memory_benchmark",
                "benchmark_failed",
                "Daemon benchmark smoke did not pass.",
                benchmark_run,
            )
        record(
            "run_memory_benchmark",
            ...
            {
                "run_id": run_id,
                "artifact_id": benchmark_run.get("artifact_id"),
                "transaction_id": benchmark_run.get("transaction_id"),
            },
        )

        benchmark_inspect = client.inspect_benchmark_run({"run_id": run_id})
        _raise_for_daemon_error("inspect_benchmark_run", benchmark_inspect)
        if benchmark_inspect.get("status") != "ok":
            raise _SmokeFailure(
                "inspect_benchmark_run",
                "benchmark_inspect_failed",
                "Daemon could not inspect the persisted benchmark run.",
                benchmark_inspect,
            )
        record("inspect_benchmark_run", "ok", {"run_id": run_id})
