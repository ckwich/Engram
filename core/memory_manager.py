...
try:
    import chromadb
except ImportError as exc:  # pragma: no cover - clean installs may omit legacy Chroma
    chromadb = None
...
# ── Limits ────────────────────────────────────────────────────────────────────
MAX_MEMORY_CHARS = MAX_DIRECT_MEMORY_CHARS
    ...
    with _chroma_owner_guard:
        if chromadb is None:
            _chroma_disabled_reason = (
                "ChromaDB legacy vector index is not installed. "
                "The daemon-owned Memory OS retrieval path uses LanceDB; "
                "legacy direct-mode Chroma recovery requires an operator-installed chromadb package."
            )
            return False
...
class MemoryManager:
    ...
    @staticmethod
    def _query_result_rows(results) -> list[tuple[Any, dict, float]]:
        """Return query result rows with usable metadata and numeric distance."""
        ...
        metadata_row = metadatas[0] if isinstance(metadatas[0], list) else []
        distance_row = distances[0] if isinstance(distances[0], list) else []
        document_row = documents[0] if documents and isinstance(documents[0], list) else []
        if not document_row:
            document_row = [None] * len(metadata_row)

        rows: list[tuple[Any, dict, float]] = []
        for document, metadata, distance in zip(document_row, metadata_row, distance_row):
            ...
            try:
                ...
                continue
            rows.append((document, metadata, normalized_distance))
        return rows
        ...
        *,
        ...
        if retrieval_mode == "hybrid":
            parts = [
                ...
                f"lexical score {lexical_score:.3f}",
            ]
        else:
            parts = [f"semantic score {semantic_score:.3f}"]
        if pinned:
    ...
    def _build_structured_payload(
        ...
        *,
        ...
        for doc, meta, distance in self._query_result_rows(raw_results):
            ...
            lexical_score = (
                lexical_relevance_score(
                    ...
                    [
                        ...
                        parent_key,
                        memory.get("title") or "",
                        memory.get("project") or "",
                        memory.get("domain") or "",
            ...
            score = (
                combine_retrieval_score(semantic_score, lexical_score)
                if normalized_mode == "hybrid"
                else semantic_score
            )
            ...
            result = {
                ...
                "title": memory["title"],
                "score": score,
                "snippet": self._make_snippet(doc or ""),
                "tags": memory["tags"],
    ...
    def _check_dedup(self, content: str, key: str) -> Optional[dict]:
        ...
        candidates = self._query_result_rows(results)
        if not candidates:
            return None
    ...
    def _parse_search_results(results) -> list[dict]:
        ...
        output = []
        for doc, meta, distance in MemoryManager._query_result_rows(results):
            ...
            output.append({
                ...
                "tags": tags,
                "score": score,
                "snippet": MemoryManager._make_snippet(doc or ""),
            })
        ...
        for path in JSON_DIR.glob("*.json"):
            try:
                ...
                memories.append({
                    ...
                    "canonical": _normalize_bool(data.get("canonical"), default=False),
    ...
    def mark_memory_reviewed(self, key: str, stale_type: str = "both") -> Optional[dict]:
        """
        Mark a memory as reviewed without deleting it.

        time: resets last_accessed.
        code: clears indexer stale flags.
        both: applies both changes.
        """
        if stale_type not in ("time", "code", "both"):
            raise ValueError("stale_type must be one of: time, code, both")
        ...
        with self._locked_json_key(key):
            ...
            saved = self._save_json(data, require_existing=True)
            if not saved:
                return None

        return {"reviewed": True, "key": key}

    def mark_memory_potentially_stale(self, key: str, reason: str = "") -> Optional[dict]:
        """Flag a memory as code-stale through the public storage interface."""
        with self._locked_json_key(key):
            data = self._load_json(key)
            if data is None:
                return None
            ...
            data["stale_reason"] = _normalize_optional_text(reason) or ""
            data["stale_flagged_at"] = _now()
            saved = self._save_json(data, require_existing=True)
            if not saved:
                return None

        return {"stale": True, "key": key}

    def clear_memory_stale_flag(self, key: str) -> Optional[dict]:
        """Clear code-stale metadata through the public storage interface."""
        with self._locked_json_key(key):
            data = self._load_json(key)
            if data is None:
                return None
