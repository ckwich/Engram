...
NULL_STATUS_LABEL = "<null>"
LEGACY_STATUS_POLICY = {
    NULL_STATUS_LABEL: {
        "memory_os_status": "active",
        "graphable": True,
        "reason": "Legacy null status is treated as active imported memory until reviewed otherwise.",
    },
    "active": {
        "memory_os_status": "active",
        "graphable": True,
        "reason": "Legacy active memories remain active.",
    },
    "accepted": {
        "memory_os_status": "accepted",
        "graphable": True,
        "reason": "Accepted Memory OS records are graphable.",
    },
    "reviewed": {
        "memory_os_status": "reviewed",
        "graphable": True,
        "reason": "Reviewed records are graphable.",
    },
    "draft": {
        "memory_os_status": "draft",
        "graphable": False,
        "reason": "Draft records are preserved but excluded from graph readiness.",
    },
    "historical": {
        "memory_os_status": "historical",
        "graphable": False,
        "reason": "Historical records are preserved but excluded from graph readiness by default.",
    },
    "current": {
        "memory_os_status": "current",
        "graphable": False,
        "reason": "Current runtime notes are preserved but not graphable unless promoted to reviewed memory.",
    },
}
...
def legacy_status_label(value: Any) -> str:
    text = _normalize_optional_text(value)
    return text if text is not None else NULL_STATUS_LABEL


def memory_os_status_from_legacy(value: Any) -> str:
    label = legacy_status_label(value)
    policy = LEGACY_STATUS_POLICY.get(label)
    if policy is None:
        return label
    return str(policy["memory_os_status"])
...
class MemoryOSMigrationKernel:
    ...
    def import_legacy_json(self, legacy_dir: str | Path, dry_run: bool = False) -> dict[str, Any]:
        ...
        duplicate_key_collisions = self._duplicate_key_collisions(records)
        if dry_run:
        ...
        if duplicate_key_collisions:
            keys = ", ".join(collision["key"] for collision in duplicate_key_collisions)
            raise ValueError(f"duplicate legacy key collision: {keys}")
        ...
        with self._connection() as conn:
            for record in records:
                ...
                if self._record_matches_ledger(conn, record):
                    replayed_count += 1
                    continue
    ...
    def _duplicate_key_collisions(self, records: list[dict[str, Any]]) -> list[dict[str, Any]]:
        grouped: dict[str, list[dict[str, Any]]] = defaultdict(list)
        for record in records:
            grouped[record["key"]].append(record)

        collisions: list[dict[str, Any]] = []
        for key, key_records in sorted(grouped.items()):
            if len(key_records) < 2:
                continue
            artifact_hashes = sorted({record["artifact_sha256"] for record in key_records})
            if len(artifact_hashes) == 1:
                continue
            collisions.append(
                {
                    "key": key,
                    "legacy_filenames": sorted(record["legacy_filename"] for record in key_records),
                    "artifact_hashes": artifact_hashes,
                }
            )
        return collisions

    def _record_matches_ledger(self, conn: sqlite3.Connection, record: dict[str, Any]) -> bool:
        row = conn.execute("SELECT * FROM memories WHERE key = ?", (record["key"],)).fetchone()
        if row is None:
            return False
        ...
        expected_memory = {
            ...
            "potentially_stale": record["potentially_stale"],
            "stale_reason": record["stale_reason"],
            "stale_flagged_at": record["stale_flagged_at"],
        }
        existing_memory = self._row_to_memory(row)
        existing_memory.pop("imported_at", None)
        if existing_memory != expected_memory:
            return False

        artifact = conn.execute(
            "SELECT legacy_filename, artifact_sha256 FROM legacy_artifacts WHERE memory_key = ?",
            (record["key"],),
        ).fetchone()
        if artifact is None:
            return False
        if artifact["legacy_filename"] != record["legacy_filename"]:
            return False
        if artifact["artifact_sha256"] != record["artifact_sha256"]:
            return False

        chunk_rows = conn.execute(
            "SELECT * FROM chunks WHERE memory_key = ? ORDER BY memory_key, chunk_index",
            (record["key"],),
        ).fetchall()
        if [self._row_to_chunk(chunk) for chunk in chunk_rows] != record["chunks"]:
            return False

        edge_rows = conn.execute(
            "SELECT * FROM graph_edges WHERE from_key = ? ORDER BY from_key, to_key, edge_id",
            (record["key"],),
        ).fetchall()
        existing_edges = [self._row_to_graph_edge(edge) for edge in edge_rows]
        expected_edges = sorted(
            record["graph_edges"],
            key=lambda edge: (_ref_key(edge["from_ref"]), _ref_key(edge["to_ref"]), edge["edge_id"]),
        )
        return existing_edges == expected_edges
    ...
    def _record_from_legacy(self, path: Path, raw_bytes: bytes, data: Any) -> dict[str, Any]:
        ...
        legacy_status = legacy_status_label(data.get("status"))
        record = {
            "key": key,
            ...
            "status": memory_os_status_from_legacy(data.get("status")),
    ...
    def _record_from_bundle_item(self, item: Any) -> dict[str, Any]:
        ...
        record = {
            ...
            "legacy_status": legacy_status_label(ledger.get("status")),
    ...
    def _build_import_report(
        ...
        legacy_status_counts = Counter(record["legacy_status"] for record in records)
        memory_os_status_counts = Counter(record["status"] for record in records)
        return {
            "schema_version": SCHEMA_VERSION,
            "dry_run": dry_run,
            ...
            "changed_count": changed_count,
            "replayed_count": replayed_count,
            "idempotent_replay": not dry_run and changed_count == 0 and replayed_count == len(records),
            ...
            "status_mapping_gaps": sorted(
                status for status in legacy_status_counts if status not in LEGACY_STATUS_POLICY
            ),
            "duplicate_key_collisions": duplicate_key_collisions,
...
def run_corpus_inventory(
    ...
    *,
    ...
    """Build a no-write legacy JSON versus Memory OS corpus inventory."""
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    corpus_inventory = subparsers.add_parser(
        "corpus-inventory",
        help="Build a no-write legacy JSON versus Memory OS corpus inventory.",
    )
    corpus_inventory.add_argument("--legacy-dir", required=True, help="Legacy data/memories directory.")
    corpus_inventory.add_argument("--memory-os-root", required=True, help="Memory OS root containing ledger.sqlite3.")
    corpus_inventory.add_argument("--report", help="Optional JSON report path.")
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "corpus-inventory":
        report = run_corpus_inventory(
            legacy_dir=args.legacy_dir,
            memory_os_root=args.memory_os_root,
            report_path=args.report,
        )
        sys.stdout.write(json.dumps(report, indent=2, ensure_ascii=False) + "\n")
        return 0
