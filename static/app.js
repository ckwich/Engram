(() => {
  ...
  async function loadInspectorTab() {
    ...
    document.getElementById('inspector-sync-list').innerHTML = '';
  ...
  function renderInspector(data) {
    ...
    const syncPanel = ((memoryOS.sync || {}).panel) || {};
    document.getElementById('inspector-sync-list').innerHTML = syncPanelRows(syncPanel).map(row => `
      <div class="usage-call-row">
        <strong>${esc(row.label)}</strong>
        <span>${esc(row.value)}</span>
        <span>${esc(row.detail)}</span>
      </div>
    `).join('') || '<div class="loading-row">No sync inspector records yet.</div>';
  ...
  function syncPanelRows(panel) {
    if (!panel || !panel.active_mode) return [];
    const hub = panel.hub || {};
    const peers = panel.peers || [];
    const health = panel.peer_direction_health || [];
    const lastExport = panel.last_export || {};
    const lastApply = panel.last_apply || {};
    const warningCodes = (panel.warnings || []).map(warning => warning.code).filter(Boolean);
    const firstHealth = health[0] || {};
    const outbound = firstHealth.outbound_lag || {};
    const inbound = firstHealth.inbound_lag || {};
    return [
      {
        label: 'Mode',
        value: panel.active_mode || 'unknown',
        detail: warningCodes.join(', ') || 'ready',
      },
      {
        label: 'Hub',
        value: hub.configured ? hub.status || 'configured' : 'not configured',
        detail: hub.auth_ready ? 'auth ready' : (hub.reachability || {}).status || 'not checked',
      },
      {
        label: 'Local device',
        value: panel.local_device_id || 'not configured',
        detail: `${safeInteger(peers.length)} peers`,
      },
      {
        label: 'Peer health',
        value: firstHealth.convergence_status || 'unknown',
        detail: `out ${outbound.last_exported_at || '-'} in ${inbound.last_applied_at || '-'}`,
      },
      {
        label: 'Last export',
        value: lastExport.exported_at || '-',
        detail: lastExport.changeset_id || '',
      },
      {
        label: 'Last apply',
        value: lastApply.applied_at || '-',
        detail: lastApply.cursor_id || '',
      },
      {
        label: 'Conflicts',
        value: safeInteger(panel.pending_conflicts),
        detail: panel.last_snapshot_id || 'no snapshot',
      },
      {
        label: 'Next',
        value: panel.rebuild_required ? 'rebuild required' : 'no rebuild',
        detail: panel.safe_next_command || '',
      },
    ];
  }
