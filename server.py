...
async def memory_protocol() -> MemoryProtocolPayload:
    ...
    return {
        ...
        "canonical_tools": {
            ...
            "query_knowledge": "Return an EKC v0 project capsule, source/document orientation, review-preparation, evidence-audit, or graph-evidence response with citations, freshness, policy, budget, planner, and typed errors.",
            ...
            "graph_backend_status": "Report JSON graph, optional Kuzu, backend config intent, migrated graph-edge, graph-parity, and daemon-readiness gates without changing live graph storage.",
            ...
            "retrieval_backend_status": "Report legacy Chroma, optional LanceDB, backend config intent, migrated-store, rebuild-probe, and golden-comparison readiness without changing live retrieval.",
        },
        "aliases": {
...
async def query_knowledge(request: dict[str, Any]) -> dict[str, Any]:
    """
    Return an Engram Knowledge Contract v0 orientation, review-preparation, evidence-audit, or graph-evidence response.
    ...
    source/document/review/audit/graph evidence, not a legacy direct-mode memory write path.
    """
    if _daemon_enabled():
        try:
