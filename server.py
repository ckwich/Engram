...
async def memory_protocol() -> MemoryProtocolPayload:
    ...
    return {
        ...
        "canonical_tools": {
            ...
            "query_knowledge": "Return an EKC v0 project capsule, source/document orientation, review-preparation, or evidence-audit response with citations, freshness, policy, budget, planner, and typed errors.",
...
async def query_knowledge(request: dict[str, Any]) -> dict[str, Any]:
    """
    Return an Engram Knowledge Contract v0 orientation, review-preparation, or evidence-audit response.
    ...
    source/document/review/audit evidence, not a legacy direct-mode memory write path.
    """
    if _daemon_enabled():
        try:
