...
- `knowledge_contract.task_types` in `memory_protocol()` must match `core.memory_os.knowledge_eval.DEFAULT_WORKFLOW_SCENARIOS`. Add an eval scenario and runtime-backed fixture before advertising a new stable EKC task type.
...
- `list_document_extractors()`, `preview_document_source_connector()`, `prepare_document_disassembly()`, `prepare_document_intake_review()`, `prepare_document_extraction_request()`, `prepare_document_extraction_result()`, `preview_document_extraction()`, `prepare_document_understanding_packet()`, `prepare_document_draft()`, `prepare_document_promotion_transaction()`, `prepare_visual_extraction_request()`, and `preview_visual_extraction()` are review/helper surfaces. They must remain no-write and must report evidence/provenance rather than promoting active memories. Visual/table/page-crop evidence must retain page number, source artifact id, coordinates/bounding boxes when available, confidence, and extractor id. `prepare_document_intake_review()` composes disassembly, text preview, quality, artifact, and missing coverage receipts into one read-only review packet; it must not promote active memory or graph edges. `prepare_visual_extraction_request()` marks visual interpretation and per-image-ref coverage as required; pass that request back to `preview_visual_extraction()` when coverage must be enforced. Understanding packets normalize agent-supplied synthesis into candidates and supplied plus auto-generated graph coverage proposals; Engram must not invent the document analysis itself or promote graph edges without review.
- `apply_document_promotion_transaction()` is a write tool, not a preview tool. It may apply selected reviewed document promotion memory and graph operations only when `accept=True` and `approved_by` is supplied, and it must route through daemon-owned Memory OS write services.
- `prepare_document_artifact_store()` and `store_document_artifact()` are the explicit ledgered document-evidence artifact path. `prepare_document_artifact_store()` records only a compact review intent, digest, and summary; `store_document_artifact()` writes only when `accept=True` and the matching reviewed packet is supplied again. These tools must not promote active memories or graph edges, and `store_document_artifact()` is not a substitute for reviewed memory promotion.
- Document intelligence ids must stay deterministic and human-readable (`doc_<slug>`, `doc_req_<source>_...`, etc.). Short digest suffixes are acceptable only as collision guards; do not introduce hash-only document or visual ids.
...
- Engram's own drafted mapping config must cover the current architecture domains: `daemon`, `memory_os`, `migration`, `document_intelligence`, `backend_status`, `graph`, `source`, `webui`, `reliability`, `codebase_mapping`, `mcp_tools`, and `legacy_adapters`.
- `prepare_codebase_mapping()` / `read_codebase_mapping_context()` / `store_codebase_mapping_result()` are agent-native. Engram prepares bounded repo context, tracks source hashes, and blocks stale stores unless forced; the connected agent performs synthesis. Do not add provider-specific model subprocesses to this path. If central files such as `server.py` or `core/memory_manager.py` are skipped by size filters, mapping payloads must warn instead of silently letting reviewers trust incomplete context.
