...
- Use `query_knowledge` for project, source, and document orientation when the thin daemon client advertises EKC v0. It is a read-only serving contract. It may read ledgered EKC artifacts and orientation records when present, but it must not be treated as permission to write or promote memory.
