...
- Use `query_knowledge` for project, source, document orientation, review preparation, and evidence audit when the thin daemon client advertises EKC v0. It is a read-only serving contract. It may read ledgered EKC artifacts, orientation records, drafts, quality receipts, and graph proposals when present, but it must not be treated as permission to write or promote memory.
