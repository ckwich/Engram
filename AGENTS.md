...
- Use `query_knowledge` for project, source, document orientation, and review preparation when the thin daemon client advertises EKC v0. It is a read-only serving contract. It may read ledgered EKC artifacts, orientation records, drafts, and quality receipts when present, but it must not be treated as permission to write or promote memory.
