...
def test_legacy_import_applies_documented_lifecycle_status_policy(tmp_path):
    legacy_dir = tmp_path / "legacy"
    store_root = tmp_path / "store"
    legacy_dir.mkdir()
    ...
    _write_memory(
        legacy_dir / "active.json",
        {
            ...
            "related_to": ["draft-memory"],
            "chunk_count": 1,
        },
    )
    _write_memory(
        legacy_dir / "draft.json",
        {
            "key": "draft-memory",
            "title": "Draft",
            "content": "Draft content",
            "status": "draft",
            "related_to": ["active-memory"],
            "chunk_count": 1,
        },
    )
    _write_memory(
        legacy_dir / "historical.json",
        {
    ...
    edges_by_key = {edge["from_ref"]["key"]: edge for edge in kernel.read_graph_edge_records()}

    assert dry_run["legacy_status_counts"] == {"<null>": 1, "draft": 1, "historical": 1}
    assert dry_run["memory_os_status_counts"] == {"active": 1, "draft": 1, "historical": 1}
    assert dry_run["status_mapping_gaps"] == []
    assert dry_run["status_policy"]["<null>"]["memory_os_status"] == "active"
    assert dry_run["status_policy"]["draft"]["graphable"] is False
    assert dry_run["status_policy"]["historical"]["graphable"] is False
    assert import_report["memory_os_status_counts"] == {"active": 1, "draft": 1, "historical": 1}
    assert kernel.read_memory_record("active-memory")["status"] == "active"
    assert kernel.read_memory_record("draft-memory")["status"] == "draft"
    assert kernel.read_memory_record("historical-memory")["status"] == "historical"
    assert edges_by_key["active-memory"]["status"] == "active"
    assert edges_by_key["draft-memory"]["status"] == "draft"
    assert edges_by_key["historical-memory"]["status"] == "historical"


def test_legacy_import_reports_idempotent_replay_without_rewriting_rows(tmp_path):
    legacy_dir = tmp_path / "legacy"
    store_root = tmp_path / "store"
    legacy_dir.mkdir()
    ...
    kernel = MemoryOSMigrationKernel(store_root)
    first = kernel.import_legacy_json(legacy_dir)
    imported_at = kernel.read_memory_record("alpha")["imported_at"]
    second = kernel.import_legacy_json(legacy_dir)
    replayed_at = kernel.read_memory_record("alpha")["imported_at"]
    ...
    assert replayed_at == imported_at


def test_legacy_import_rejects_duplicate_key_collisions(tmp_path):
    legacy_dir = tmp_path / "legacy"
    store_root = tmp_path / "store"
    legacy_dir.mkdir()

    _write_memory(
        legacy_dir / "first.json",
        {
            "key": "duplicate",
            "title": "First",
    ...
    assert len(dry_run["duplicate_key_collisions"]) == 1
    collision = dry_run["duplicate_key_collisions"][0]
    assert collision["key"] == "duplicate"
    assert collision["legacy_filenames"] == ["first.json", "second.json"]
    assert len(collision["artifact_hashes"]) == 2
    assert len(set(collision["artifact_hashes"])) == 2
    with pytest.raises(ValueError, match="duplicate legacy key collision"):
...
def test_migration_cli_writes_corpus_inventory_report(tmp_path):
    legacy_dir = tmp_path / "legacy"
    memory_os_root = tmp_path / "memory_os"
    report_path = tmp_path / "corpus_inventory.json"
    legacy_dir.mkdir()
    _write_memory(
        legacy_dir / "alpha.json",
        {
    ...
    assert report["legacy_json_count"] == 1
