...
def test_generated_git_hook_uses_json_path_literals(tmp_path):
    import engram_index

    project = tmp_path / "repo & spaces"
    (project / ".git" / "hooks").mkdir(parents=True)

    engram_index.run_install_hook(project)

    hook_text = (project / ".git" / "hooks" / "post-commit").read_text(encoding="utf-8")
    assert f"PROJECT_ROOT = {json.dumps(str(project))}" in hook_text
    assert "PROJECT_ROOT = r" not in hook_text
...
def test_compose_raw_daemon_port_is_loopback_published():
    compose = Path("compose.yml").read_text(encoding="utf-8")

    assert 'ENGRAM_DAEMON_ALLOWED_HOSTS: engramd-core,127.0.0.1,localhost' in compose
    assert '- "127.0.0.1:${ENGRAM_DAEMON_HOST_PORT:-8765}:8765"' in compose
    assert '- "${ENGRAM_DAEMON_HOST_PORT:-8765}:8765"' not in compose
