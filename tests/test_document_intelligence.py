...
def test_preview_visual_extraction_requires_coverage_for_every_requested_image_ref():
    ...
    assert preview["visual_coverage"]["visual_request_id"] == request["request_id"]
    assert preview["visual_coverage"]["required_image_ref_count"] == 2
    assert preview["visual_coverage"]["covered_image_ref_count"] == 2
    assert preview["visual_coverage"]["missing_image_refs"] == []
    assert preview["visual_coverage"]["missing_capabilities"] == []
    assert preview["visual_coverage"]["coverage_complete"] is True
...
def test_preview_visual_extraction_honors_per_image_ref_capabilities():
    document = prepare_document_record(
        title="Mixed Coverage Book",
        source_uri="file:///docs/mixed-coverage.pdf",
        source_type="pdf",
        content_hash="sha256:" + "a" * 64,
        media_type="application/pdf",
    )
        ...
        observations=[
            {
                ...
                "source_ref": request["image_refs"][0],
                "page_number": 4,
                "text": "OCR text for the low-text page.",
                "confidence": 0.91,
            },
            {
                "artifact_type": "table",
                "source_ref": request["image_refs"][1],
                "page_number": 5,
                "description": "A reviewed comparison table.",
                "confidence": 0.9,
            },
        ],
        extractor_id="agent-native-vision",
    ...
    assert request["image_refs"][0]["requested_capabilities"] == ["ocr_text"]
    assert request["image_refs"][1]["requested_capabilities"] == ["table_structure"]
    assert preview["status"] == "ok"
    assert preview["quality_warnings"] == []
    assert preview["visual_coverage"]["missing_capabilities"] == []
    assert preview["visual_coverage"]["coverage_complete"] is True
