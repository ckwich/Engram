...
def test_retrieval_eval_api_returns_report(monkeypatch):
    ...
    client = webui.app.test_client()
    response = client.post("/api/eval/retrieval")

    assert response.status_code == 200
    assert response.get_json()["summary"]["passed"] is True


def test_retrieval_eval_api_rejects_get_without_running_eval(monkeypatch):
    import webui
    ...
    response = webui.app.test_client().get("/api/eval/retrieval")
