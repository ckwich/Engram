...
def test_snapshot_manifest_is_explicitly_not_restore_grade(tmp_path):
    ledger = MemoryOSLedger(tmp_path / "engram.sqlite")
    service = SnapshotService(ledger)

    snapshot = service.create_snapshot(created_by="agent")

    assert snapshot["schema_version"] == "2026-05-22.snapshot-manifest.v1"
    assert snapshot["record_type"] == "snapshot_manifest"
    assert snapshot["snapshot_kind"] == "manifest_only"
    assert snapshot["restore_grade"] is False
    assert snapshot["rollback_supported"] is False
    assert snapshot["rollback_semantics"]["status"] == "not_implemented"
    assert snapshot["rollback_semantics"]["safe_use"] == "diff_replay_and_operator_review"
    assert "not a restore point" in snapshot["operator_guidance"]
    assert "durable_payloads_captured" in snapshot["limitations"]
