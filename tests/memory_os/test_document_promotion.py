...
def test_apply_document_promotion_writes_selected_memory_only(tmp_path):
    ...
    assert result["automatic_graph_edges_written"]
    ...
    } == {"memory_metadata_graph", "memory_semantic_graph"}
...
def test_apply_document_promotion_batches_selected_graph_edges(tmp_path):
    runtime = _runtime(tmp_path)
    transaction = _promotion_transaction()
    second_edge_operation = {
        **transaction["operations"][1],
        "payload": {
            **transaction["operations"][1]["payload"],
            "to_ref": {"kind": "concept", "key": "batched_graph_import"},
        },
    }
    transaction = {
        **transaction,
        "operations": [*transaction["operations"], second_edge_operation],
    }
    ...
    class RecordingGraph:
        ...
        def import_edges(self, edges):
            self.calls.append(list(edges))
            return self.graph.import_edges(edges)

    recording_graph = RecordingGraph(original_graph)
    ...
    result = apply_document_promotion_transaction(
        ...
        accept=True,
        approved_by="agent-review",
        selected_operation_indexes=[1, 2],
    )

    assert result["status"] == "ok"
    assert result["memories_written"] == []
    assert len(result["graph_edges_written"]) == 2
    assert len(recording_graph.calls) == 1
    assert [edge["edge_id"] for edge in recording_graph.calls[0]] == result["graph_edges_written"]
