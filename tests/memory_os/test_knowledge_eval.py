...
EXPECTED_STABLE_EKC_TASK_TYPE_ORDER = [
    "project_orientation",
    "source_orientation",
...
EXPECTED_STABLE_EKC_TASK_TYPES = set(EXPECTED_STABLE_EKC_TASK_TYPE_ORDER)
EXPECTED_WORKFLOW_SCENARIO_IDS = {
    *EXPECTED_STABLE_EKC_TASK_TYPES,
    "document_intelligence_ingestion",
    "knowledge_pr_review_state",
    "memory_benchmark_smoke",
}
...
class FakeRuntime:
    ...
    def list_memory_benchmark_suites(self):
        return {
    ...
    def run_memory_benchmark(self, *, suite_id="smoke", seed=42, persist=True):
        return {
            "schema_version": "2026-05-26.memory-benchmark.v1",
            "run_id": "benchmark_run:fake_smoke",
            "suite_id": suite_id,
            "seed": seed,
            "summary": {"status": "pass", "scenario_count": 4, "passed": 4, "failed": 0},
            "write_performed": bool(persist),
            "active_memory_write_performed": False,
            "graph_write_performed": False,
            "error": None,
        }

    def inspect_benchmark_run(self, *, run_id):
        return {
            "schema_version": "2026-05-26.memory-benchmark.v1",
            "status": "ok",
            "run_id": run_id,
    ...
    def prepare_document_ingestion_plan(self):
        return {
            "status": "callable",
            "write_performed": False,
            "active_memory_write_performed": False,
    ...
    def run_document_ingestion(self):
        return {
            "status": "callable",
            "write_performed": False,
            "active_memory_write_performed": False,
    ...
    def inspect_document_ingestion(self):
        return {
            "status": "callable",
            "write_performed": False,
            "active_memory_write_performed": False,
...
def test_knowledge_contract_eval_covers_stable_1_0_workflows():
    ...
    assert set(STABLE_EKC_TASK_TYPES) == EXPECTED_STABLE_EKC_TASK_TYPES
    assert list(STABLE_EKC_TASK_TYPES) == EXPECTED_STABLE_EKC_TASK_TYPE_ORDER
    scenario_ids = [scenario["scenario_id"] for scenario in DEFAULT_WORKFLOW_SCENARIOS]
    assert len(scenario_ids) == len(set(scenario_ids))
    assert set(scenario_ids) == EXPECTED_WORKFLOW_SCENARIO_IDS
    assert {
        scenario["task_type"]
        for scenario in DEFAULT_WORKFLOW_SCENARIOS
    } <= EXPECTED_STABLE_EKC_TASK_TYPES
    ...
    assert report["workflow_coverage"]["required_methods_covered_rate"] == 1.0
    for row in report["workflow_coverage"]["scenarios"]:
        ...
        if row["required_methods"]:
            assert row["required_methods_covered"] is True
        if row["scenario_id"] == "memory_benchmark_smoke":
            run_result = next(
                result
                for result in row["required_method_results"]
                if result["method"] == "run_memory_benchmark"
            )
            inspect_result = next(
                result
                for result in row["required_method_results"]
                if result["method"] == "inspect_benchmark_run"
            )
            assert run_result["write_performed"] is False
            assert inspect_result["status"] == "skipped"
...
def test_workflow_scenarios_include_document_intelligence_ingestion_gate():
    scenarios = {
        scenario["scenario_id"]: scenario
        for scenario in DEFAULT_WORKFLOW_SCENARIOS
    }

    scenario = scenarios["document_intelligence_ingestion"]

    assert scenario["task_type"] == "document_orientation"
    assert scenario["required_methods"] == [
...
def test_workflow_scenarios_include_knowledge_pr_review_state_without_new_task_type():
    scenarios = {
        scenario["scenario_id"]: scenario
        for scenario in DEFAULT_WORKFLOW_SCENARIOS
    }

    scenario = scenarios["knowledge_pr_review_state"]

    assert scenario["task_type"] == "review_preparation"
    assert scenario["focus"] == ["knowledge pr", "memory ci"]
    assert "knowledge_pr_review_state" not in STABLE_EKC_TASK_TYPES


def test_workflow_scenarios_include_memory_benchmark_smoke_gate():
    scenarios = {
        scenario["scenario_id"]: scenario
        for scenario in DEFAULT_WORKFLOW_SCENARIOS
    }

    scenario = scenarios["memory_benchmark_smoke"]

    assert scenario["task_type"] == "evidence_audit"
    assert scenario["focus"] == ["benchmark", "memory ci"]
    assert scenario["required_methods"] == [
        "list_memory_benchmark_suites",
        "run_memory_benchmark",
        "inspect_benchmark_run",
    ]
    assert "memory_benchmark_smoke" not in STABLE_EKC_TASK_TYPES
