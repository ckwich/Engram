...
def test_public_host_blocks_retrieval_eval_without_write_token(monkeypatch):
    import webui
...
def test_public_host_allows_retrieval_eval_with_write_token(monkeypatch):
    import webui
...
def test_dashboard_retrieval_eval_call_uses_post_and_write_headers():
    script = Path("static/app.js").read_text(encoding="utf-8")

    eval_call_start = script.index("fetch('/api/eval/retrieval'")
    eval_call = script[eval_call_start : script.index("});", eval_call_start) + 3]

    assert "method: 'POST'" in eval_call
    assert "headers: getWriteHeaders()" in eval_call
