...
def test_usage_paths_are_data_root_relative():
    import core.usage_meter as usage_meter_module

    assert usage_meter_module.USAGE_DIR == resolve_data_root() / "usage"
...
def test_record_tool_call_accepts_daemon_telemetry_fields(isolated_usage_meter):
    meter = isolated_usage_meter.usage_meter

    event = meter.record_tool_call(
        tool="search_memories",
        input_payload={"query": "daemon telemetry"},
        output_payload={"results": [{"key": "runtime_memory"}]},
        status="ok",
        duration_ms=9,
        telemetry={
            "entrypoint": "engramd",
            "backend_used": "memory_os",
            "fallback_used": False,
