...
def test_memory_protocol_advertises_document_coverage_workbench():
    server = load_server_module()

    payload = asyncio.run(server.memory_protocol())

    assert "prepare_document_coverage_workbench" in payload["tool_groups"]["document_intelligence"]["tools"]
    assert payload["progressive_discovery"]["load_next"]["document coverage workbench"] == "prepare_document_coverage_workbench"
    assert "page-render/OCR/table" in payload["canonical_tools"]["prepare_document_coverage_workbench"]
    ...
    assert "sanitized job-event receipt" in payload["canonical_tools"]["prepare_document_coverage_pass"]


def test_memory_protocol_advertises_knowledge_prs_and_memory_ci():
    server = load_server_module()

    payload = asyncio.run(server.memory_protocol())

    assert payload["tool_groups"]["knowledge_prs"]["tools"] == [
    ...
    assert "explicit accept=True" in payload["canonical_tools"]["merge_knowledge_pr"]


def test_document_ingestion_tools_route_to_daemon_with_stable_payloads(monkeypatch):
    server = load_server_module()
    calls: list[tuple[str, dict[str, object]]] = []

    async def fake_call_daemon(method_name: str, payload: dict[str, object]) -> dict[str, object]:
        calls.append((method_name, payload))
        return {"status": "ok", "method": method_name}

    monkeypatch.setattr(server, "_daemon_enabled", lambda: True)
    monkeypatch.setattr(server, "_call_daemon", fake_call_daemon)

    assert (
        asyncio.run(
            server.prepare_document_ingestion_plan(
    ...
    assert (
        asyncio.run(
            server.resume_document_ingestion(
                ingestion_id="doc_ingest_book",
                accept=True,
                approved_by="agent-review",
    ...
    assert (
        asyncio.run(server.inspect_document_ingestion(ingestion_id="doc_ingest_book", document_id="doc_book"))[
            "method"
        ]
    ...
    for _, payload in calls:
        ...
        assert "selected_operation_indexes" not in payload


def test_document_ingestion_tools_require_daemon_in_direct_mode(monkeypatch):
    server = load_server_module()
    monkeypatch.setattr(server, "_daemon_enabled", lambda: False)

    payloads = [
        asyncio.run(server.prepare_document_ingestion_plan("/docs/book.pdf")),
        asyncio.run(server.run_document_ingestion("doc_ingest_book", accept=True, approved_by="agent-review")),
        asyncio.run(server.resume_document_ingestion("doc_ingest_book", accept=True, approved_by="agent-review")),
        asyncio.run(server.inspect_document_ingestion(ingestion_id="doc_ingest_book")),
    ]
    ...
    for payload in payloads:
        ...
        assert payload["graph_write_performed"] is False
        assert payload["error"]["code"] == "daemon_required"
...
def test_list_document_extractors_tool_returns_no_write_catalog():
    ...
    assert any(
        extractor["id"] == "engram-local-coverage-workbench"
        and extractor["runs_inside_engram"] is True
        and "prepare_document_coverage_workbench" in extractor["next_tools"]
        for extractor in payload["catalog"]["extractors"]
    )
...
def test_prepare_document_coverage_workbench_tool_returns_no_write_packet(tmp_path, monkeypatch):
    server = load_server_module()
    pdf = tmp_path / "sample.pdf"
    pdf.write_bytes(b"%PDF-1.7\n% sample")
    observed: dict[str, object] = {}
    ...
    monkeypatch.setattr(server, "build_document_coverage_workbench", fake_workbench)

    payload = asyncio.run(
        server.prepare_document_coverage_workbench(
            source_path=str(pdf),
            document_record={"document_id": "doc_sample"},
            visual_request={"request_id": "vis_req_sample", "document_id": "doc_sample", "image_refs": []},
    ...
    assert payload["error"] is None
    assert payload["workbench"]["record_type"] == "document_coverage_workbench"
    assert payload["workbench"]["write_performed"] is False
    assert observed["source_path"] == str(pdf)
    assert observed["render_pages"] is False
    assert observed["run_ocr"] is False
    assert observed["run_table_detection"] is False


def test_prepare_document_coverage_workbench_routes_to_daemon(monkeypatch):
    server = load_server_module()
    calls: list[tuple[str, dict[str, object]]] = []

    async def fake_call_daemon(method_name: str, payload: dict[str, object]) -> dict[str, object]:
        calls.append((method_name, payload))
        return {"workbench": {"status": "ok"}, "error": None}
    ...
    payload = asyncio.run(
        server.prepare_document_coverage_workbench(
            source_path="/docs/book.pdf",
            document_record={"document_id": "doc_book"},
            visual_request={"request_id": "vis_req_book", "document_id": "doc_book", "image_refs": []},
...
def test_store_prepared_memory_guardrail_preflight_blocks_partial_legacy_promotion(monkeypatch):
    server = load_server_module()
    ...
    payload = asyncio.run(server.store_prepared_memory("draft-a", selected_items=[0, 1]))

    assert payload["stored_count"] == 0
    assert payload["stored"] == []
    assert payload["error"]["code"] == "memory_guardrail_blocked"
    assert payload["skipped"][0]["key"] == "secret_source_memory"
    assert stored == []
